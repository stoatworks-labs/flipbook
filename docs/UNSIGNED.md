# Running unsigned Flipbook builds
...
```sh
xattr -dr com.apple.quarantine ~/Documents/"Resolume Arena"/"Extra Effects"/Flipbook.bundle
```
...
```sh
codesign --force --deep --sign - ~/Documents/"Resolume Arena"/"Extra Effects"/Flipbook.bundle
```

Verify it took:

```sh
codesign -dv --verbose=4 ~/Documents/"Resolume Arena"/"Extra Effects"/Flipbook.bundle
spctl -a -vv ~/Documents/"Resolume Arena"/"Extra Effects"/Flipbook.bundle   # still reports "rejected" — ad-hoc is not notarization
```
